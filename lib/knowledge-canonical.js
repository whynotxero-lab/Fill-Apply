...
(function (global) {
  ...
  var FIELD_TYPES = ['text', 'boolean', 'number', 'select', 'multi-select', 'date', 'url'];
  ...
  function matchCanonical(label, extras) {
    ...
    return {
      ...
      source: 'derived',
      fieldType: extras && extras.fieldType ? extras.fieldType : 'text',
      profileKey: null,
      derived: true
    };
  }
  ...
  function inferFieldType(el, descriptor) {
    ...
    if (el && el.tagName === 'SELECT' && el.multiple) return 'multi-select';
    if (el && el.tagName === 'SELECT') return 'select';
    if (type === 'select-one') return 'select';
    if (type === 'select-multiple') return 'multi-select';
    return 'text';
  }

  function normalizeValue(value, fieldType) {
    if (value == null) return '';
    if (fieldType === 'multi-select' && Array.isArray(value)) {
  ...
  function isValidFieldType(t) {
    return FIELD_TYPES.indexOf(t) !== -1;
  }
