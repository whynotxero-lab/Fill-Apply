...
(function (global) {
  ...
  function aliasScore(label, aliases) {
    ...
    (aliases || []).forEach(function (alias) {
      ...
      if (hay === q) {
        ...
        // Full alias appears inside the question — strong
  ...
  /** Evidence layers — higher priority wins; lower cannot override. */
  ...
  function matchWording(text, extras) {
    ...
    // Competing near-tied keys → AMBIGUOUS
    ...
    if (uniqKeys.length > 1) {
      // Prefer non-excluded clear winner only if one score dominates
   ...
   * Priority (higher wins; lower cannot override / create conflicting identity):
   *   1 question / label → 2 ariaLabel → 3 groupContext → 4 placeholder
   *   → 5 autocomplete → 6 name/id
   * Strong conflict / ambiguous bare wording → AMBIGUOUS (DO_NOT_FILL).
  ...
  function resolveFromEvidence(evidence, extras) {
    ...
    for (var p = 0; p < primaryTexts.length; p++) {
      ...
      if (prim.ambiguous) {
        return {
          ...
          ambiguous: true,
          action: 'DO_NOT_FILL',
          reason: (prim.hit && prim.hit.reason) || 'ambiguous',
          matchedEvidence: prim.layer,
          candidateKeys: (prim.hit && prim.hit.candidateKeys) || candidateKeys,
          evidence: evidence
        };
      }
      if (!winner) {
        winner = prim.hit;
        matchedEvidence = prim.layer;
      } else if (prim.hit.key !== winner.key && areCompetingKeys(prim.hit.key, winner.key)) {
        ...
        return {
          ...
          candidateKeys: [winner.key, prim.hit.key],
          evidence: evidence
        };
      }
    }

    // Priority 2–5: aria, group, placeholder, autocomplete — only if no winner yet
    // (lower cannot override). If winner exists, still scan for diagnostics.
    var secondary = [
    ...
    for (var s = 0; s < secondary.length; s++) {
      var sec = consider(secondary[s].layer, secondary[s].text, { allowDerive: secondary[s].allowDerive });
      if (!sec || sec.ambiguous) {
        if (sec && sec.ambiguous && !winner) {
          return {
            key: null,
            semanticKey: null,
            score: 0,
            ...
            ambiguous: true,
            action: 'DO_NOT_FILL',
            reason: (sec.hit && sec.hit.reason) || 'ambiguous',
            matchedEvidence: sec.layer,
            candidateKeys: (sec.hit && sec.hit.candidateKeys) || candidateKeys,
            evidence: evidence
          };
        }
        continue;
      ...
      // else: lower cannot override — ignore conflicting key
    }

    // Priority 6: name / id — NEVER create permanent identity from opaque DOM ids.
    // Only match known catalog / learned keys (allowDerive: false). Cannot override.
    var domLayers = [
    ...
    for (var d = 0; d < domLayers.length; d++) {
      ...
      if (!raw) continue;
      // Humanize snake/camel for catalog match only
      var humanized = String(raw).replace(/([a-z])([A-Z])/g, '$1 $2').replace(/[_-]+/g, ' ');
      var dom = consider(domLayers[d].layer, humanized, { allowDerive: false, allowAmbiguousBare: false });
      if (!dom || dom.ambiguous) continue;
      if (!winner) {
        // DOM-only identity: accept catalog/learned match, but never derived opaque ids
        if (dom.hit.source === 'catalog' || dom.hit.source === 'learned') {
          winner = dom.hit;
          matchedEvidence = domLayers[d].layer;
        }
      }
      // Conflicting name/id vs higher-priority winner: higher wins (concrete cases)
    }
