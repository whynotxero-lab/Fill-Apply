...
(function (global) {
  ...
  const FIELD_MAP = [
    ...
    },
    {
      ...
      placeholders: ['degree', 'mba', 'qualification title']
    },
    {
      key: 'fieldOfStudy',
      autocomplete: [],
    ...
    },
    {
      ...
      placeholders: ['yyyy', 'graduation year', 'year']
    },
    {
      key: 'gpa',
      autocomplete: [],
  ...
  function scoreMatch(entry, descriptor) {
    ...
    (entry.names || []).forEach(function (p) {
      ...
      function nameHits(token) {
        ...
        if (token.length >= 5 && pn.includes(token)) {
          if (/^(title|name|code|type|date|city|role|country|year)$/.test(token)) return false;
          return true;
        }
        return false;
      }
    ...
    if (
      (entry.key === 'title' || entry.key === 'salutation') &&
      /\b(current|job|position|role|designation|qualification|degree|course)\b/.test(lab)
    ) {
      score = 0;
    }
    ...
    // Qualification Title / Degree Title → degree, NEVER honorific Mr/Ms
    if (/qualification\s*title|degree\s*title|title\s*of\s*(the\s*)?(degree|qualification)|course\s*title/.test(lab)) {
      if (entry.key === 'degree' || entry.key === 'fieldOfStudy' || entry.key === 'highestEducation') {
        score += 160;
      }
      if (entry.key === 'title' || entry.key === 'salutation' || entry.key === 'currentTitle') {
        score = 0;
      }
    }
    // Country / Region of Residence (and employment/education country) ≠ dial code
    if (
      /of\s*residence|residence\s*country|residential|employment\s*country|education\s*country|country\s*\/?\s*region\s*of\s*residence/.test(
        lab
      ) ||
      (lab === 'country / region' || lab === 'country/region')
    ) {
      if (entry.key === 'country' || entry.key === 'countryOfResidence' || entry.key === 'addressCountry') {
        score += 160;
      }
      if (entry.key === 'phoneCountry') score = 0;
    }
    // Bare "Year" in education → graduationYear; never degree string
    if (lab === 'year' || lab === 'year *' || lab === 'end year') {
      if (entry.key === 'graduationYear' || entry.key === 'birthYear') score += 80;
      if (entry.key === 'degree' || entry.key === 'title' || entry.key === 'salutation') score = 0;
    }
  ...
  function bestKeyForField(descriptor, minScore) {
    ...
    let best = null;
    let bestScore = 0;
    var SI = typeof globalThis !== 'undefined' ? globalThis.FillApplyScreeningIntent : null;
    var screenIntent = SI && SI.classify ? SI.classify((descriptor && descriptor.label) || '') : null;
    for (let i = 0; i < FIELD_MAP.length; i++) {
      const entry = FIELD_MAP[i];
      let s = scoreMatch(entry, descriptor);
      if (screenIntent && SI && SI.INTENT) {
        var essayish =
          screenIntent === SI.INTENT.IFRS_TAX ||
          screenIntent === SI.INTENT.SOX_AUDIT ||
          screenIntent === SI.INTENT.BANKING_FS ||
          screenIntent === SI.INTENT.EXPERIENCE_ESSAY;
        if (essayish && /^(city|location|country|state|zip|noticePeriod|availableFrom)$/.test(entry.key)) {
          s = 0;
        }
        if (screenIntent === SI.INTENT.UAE_BASED && /^(city|location|noticePeriod|availableFrom)$/.test(entry.key)) {
          s = 0;
        }
        if (
          (screenIntent === SI.INTENT.SALARY_CURRENT ||
            screenIntent === SI.INTENT.SALARY_EXPECTED ||
            screenIntent === SI.INTENT.SALARY_GENERIC) &&
          entry.key === 'noticePeriod'
        ) {
          s = 0;
        }
        if (screenIntent === SI.INTENT.NOTICE && /^(city|location|currentSalary|expectedSalary|salaryText)$/.test(entry.key)) {
          s = 0;
        }
      }
