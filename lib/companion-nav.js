...
(function (global) {
  ...
  var COMPANION_NAV_CTA =
    /\b(save\s*(&|and)\s*continue|continue(\s+application)?|next(\s*step)?|proceed|start\s*application|begin\s*application|start\s*apply|apply\s*now|easy\s*apply)\b/i;

  /** Hard exclusions — never final-submit / back / paid upsell. Bare Apply allowed when nav-only. */
  var COMPANION_NAV_EXCLUDE =
    /\b(back|previous|cancel|close|dismiss|submit(\s+application)?|send\s+application|mark\s*as\s*applied|applied\s*successfully|open\s*application|auto[- ]?apply|upgrade|subscribe)\b/i;
  ...
  function scoreCompanionNav(el, text) {
    ...
    else if (/apply\s*now|easy\s*apply|^apply$/i.test(t)) s += 65;
