...
(function (global) {
  ...
  async function load() {
    ...
    list = list.map(normalize).filter(Boolean);
    // Empty storage must not wipe in-memory authored answers (Options add in
    // same session, or environments without chrome.storage). When storage has
    // rows, storage wins (fresh content-script inject after Options save).
    if (!list.length) {
      return listRecords();
    }
    memory.records = list;
