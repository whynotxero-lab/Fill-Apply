...
(function (global) {
  ...
  var APPLY_START_CTA =
    /\b(open\s*application|apply\s*now|start\s*apply|start\s*application|begin\s*application|continue\s*application|easy\s*apply|apply\s*for\s*this\s*(job|role|position)|apply\s*to\s*this\s*(job|role)|apply\s*here|apply)\b/i;
  ...
  var EXCLUDED_APPLY_CTA =
    /\b(ai\s*auto[- ]?apply|auto[- ]?apply\s*with\s*ai|auto[- ]?apply|upgrade|subscribe|pricing|premium|share(\s*job)?|save\s*job|bookmark|tailor\s*my\s*resume|post\s*a\s*job|hire\s*(talent|now)?|mark\s*as\s*applied|applied\s*successfully|application\s*issue|blocked)\b/i;
  ...
  function isExcludedApplyCta(text) {
    ...
    return EXCLUDED_APPLY_CTA.test(t);
  }
