...
(function (global) {
  ...
  /**
   * Host-aware Apply-start: Glassdoor/Indeed/NaukriGulf treat Easy Apply as a valid
   * start CTA (their board adapters also use findEasyApplyButton). Other hosts keep
   * Easy Apply excluded (LinkedIn owns that path).
   */
  function isApplyStartCtaForHost(text, host) {
    var h = String(host || '').toLowerCase();
    if (/glassdoor\.com|indeed\.com|naukrigulf\.com/.test(h) && isEasyApplyCta(text)) {
      return true;
    }
