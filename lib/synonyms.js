...
(function (global) {
  ...
  function findApplyStartButtons(root) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      var t = buttonText(el);
      if (!isApplyStartCta(t)) continue;
      out.push(el);
    }
    out.sort(function (a, b) {
      return scoreApplyStartText(buttonText(b)) - scoreApplyStartText(buttonText(a));
    });
    return out;
  }
  ...
  function isApplicationFormOpen(root, minFields) {
    ...
    // Greenhouse-style: #application present with real fields
    var gh = root.querySelector(
      '#application, #application-form, form#application, #greenhouse-job-application, #main_fields, .application--container, #apply_form'
    );
    if (gh && countFillableApplicationInputs(gh) >= minFields) return true;

    var n = countFillableApplicationInputs(root);
    if (n >= minFields) return true;

    // File + email often means apply form even with few text fields
    try {
      var hasFile = !!root.querySelector('input[type="file"]');
      var hasEmail = !!root.querySelector(
        'input[type="email"], input[name*="email" i], input[id*="email" i], input[autocomplete="email"]'
      );
      if (hasFile && hasEmail && n >= 1) return true;
    } catch (_e2) {}
  ...
  function shouldClickApplyStart(root, minFields) {
    ...
    // Prefer when page looks like a posting OR has almost no fillable fields
    var n = countFillableApplicationInputs(root);
    if (n < (minFields == null ? MIN_OPEN_FORM_FIELDS : minFields)) return true;
    if (looksLikeJobPosting(root)) return true;
    // Greenhouse: Apply for this job before #application fields exist
    try {
