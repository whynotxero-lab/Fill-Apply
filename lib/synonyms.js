...
(function (global) {
  ...
  var APPLY_START_CTA =
    /\b(open\s*application|apply\s*now|start\s*apply|start\s*application|begin\s*application|continue\s*application|easy\s*apply|apply\s*manually|autofill\s*with\s*resume|apply\s*with\s*(cv|resume|curriculum)|apply\s*for\s*this\s*(job|role|position)|apply\s*to\s*this\s*(job|role)|apply\s*here|apply)\b/i;
  ...
  var MARK_APPLIED_CTA = /^(mark\s*as\s*applied|applied\s*successfully)$/i;
  ...
  var EXCLUDED_APPLY_CTA =
    /\b(ai\s*auto[- ]?apply|auto[- ]?apply\s*with\s*ai|auto[- ]?apply|upgrade|subscribe|pricing|premium|share(\s*job)?|save\s*job|bookmark|shortlist|add\s*to\s*shortlist|tailor\s*my\s*resume|post\s*a\s*job|hire\s*(talent|now)?|mark\s*as\s*applied|applied\s*successfully|application\s*issue|blocked)\b/i;

  /**
   * Location / country / search-filter chips (Bayt etc.) — never Apply-start.
   * Pure country/city names, or filter chrome without an Apply verb.
   */
  var LOCATION_FILTER_CHIP_CTA =
    /^(saudi\s*arabia|united\s*arab\s*emirates|u\.\s*a\.\s*e\.?|uae|qatar|kuwait|bahrain|oman|egypt|jordan|lebanon|pakistan|india|philippines|riyadh|jeddah|dammam|khobar|dubai|abu\s*dhabi|sharjah|doha|manama|muscat|cairo|remote|hybrid|on[- ]?site|full[- ]?time|part[- ]?time|location|country|city|nationality|filter|filters|clear\s*filters?|all\s*locations?)$/i;

  /**
   * JobPool page chrome: sidebar / menu label "Fill-Apply" matches \\bapply\\b but is
   * NOT an Apply-start CTA. Exact page-nav titles only (not "Fill-Apply queue").
   */
  var JOBPOOL_NAV_CHROME_CTA = /^(fill[- ]?apply|applications?|pipeline(\s+overview)?|logs?|documents?|settings?)$/i;
  ...
  function isExcludedApplyCta(text) {
    ...
    if (!t) return false;
    if (JOBPOOL_NAV_CHROME_CTA.test(t)) return true;
    if (EXCLUDED_APPLY_CTA.test(t)) return true;
    if (LOCATION_FILTER_CHIP_CTA.test(t)) return true;
    // Workday social option — Workday adapter owns LinkedIn path; generic start excludes it
    if (/apply with linkedin/i.test(t)) return true;
    if (/^use my last application$/i.test(t)) return true;
    // Country / city search chip without Apply verb (e.g. Bayt "Saudi Arabia")
    if (
      /^(saudi|arabia|emirates|uae|qatar|kuwait|bahrain|oman|egypt|jordan|pakistan|india)\b/i.test(t) &&
      !/\bapply\b/i.test(t)
    ) {
      return true;
    }
  ...
  /**
   * True when el is left-nav / sidebar chrome (or href to /fill-apply while already there).
   * Used so Auto Apply never re-clicks the page button labeled "Fill-Apply".
   */
  function isNavigationChrome(el, pageUrl) {
    if (!el) return false;
    try {
      var t = buttonText(el).replace(/\s+/g, ' ').trim();
      if (JOBPOOL_NAV_CHROME_CTA.test(t)) return true;
      var stamped =
        hasDataFillApply(el, DATA_APPLY_START) ||
        hasDataFillApply(el, DATA_JOBPOOL_APPLY);
      var isOpenApp = /^open\s*application$/i.test(t) || /\bopen\s*application\b/i.test(t);
      if (el.closest) {
        var navAnc = el.closest(
          'nav, [role="navigation"], aside, [class*="sidebar" i], [class*="side-nav" i], [data-sidebar], [data-slot="sidebar"]'
        );
        if (navAnc) {
          // Inside nav: exclude unless stamped Open Application / Apply-start
          if (isOpenApp || stamped) return false;
          return true;
        }
      }
      var href = '';
      try {
        href = String(el.href || (el.getAttribute && el.getAttribute('href')) || '');
      } catch (_h) {
        href = '';
      }
      if (/\/fill-apply(?:\/|$|\?)/i.test(href) && !isOpenApp && !stamped) {
        var cur = String(
          pageUrl ||
            (typeof location !== 'undefined' && location.href) ||
            ''
        );
        if (/\/fill-apply(?:\/|$|\?)/i.test(cur)) return true;
      }
    } catch (_e) {}
    return false;
  }
  ...
  function isApplyStartCtaForHost(text, host) {
    ...
    if (/glassdoor\.com|indeed\.com|naukrigulf\.com|bayt\.com/.test(h) && isEasyApplyCta(text)) return true;
  ...
  function scoreApplyStartText(t) {
    ...
    if (!t || isExcludedApplyCta(t)) return 0;
    if (!isApplyStartCta(t) && !isEasyApplyCta(t) && !/apply with (cv|resume)/i.test(t)) return 0;
    // JobPool Fill-Apply hub primary CTA
    if (/^open\s*application$/i.test(t)) return 110;
    if (/\bopen\s*application\b/i.test(t)) return 105;
    if (/^apply now$/i.test(t)) return 100;
    if (/^APPLY$/i.test(t) && t.length <= 8) return 99;
    if (/^apply manually$/i.test(t)) return 99;
    if (/autofill with resume/i.test(t)) return 97;
    if (/apply with (cv|resume|curriculum)/i.test(t)) return 98;
    if (/^start\s*apply$/i.test(t)) return 97;
  ...
  function findApplyStartButtons(root) {
    ...
    try {
      host =
        ...
        (root.defaultView && root.defaultView.location && root.defaultView.location.hostname) ||
        '';
    } catch (_h) {
      host = '';
    }
    function isStart(text) {
      return isApplyStartCtaForHost(text, host) || /apply with (cv|resume|curriculum)/i.test(String(text || ''));
    }
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (isNavigationChrome(el)) continue;
      if (highConfidenceOverlay && (!t || !isStart(t))) {
      ...
      if (!markedStart && !isStart(t) && !highConfidenceOverlay) continue;
      out.push(el);
    }
  ...
  function findContinueButtons(root) {
    ...
    out.sort(function (a, b) {
      ...
      if (/save\s*(&|and)\s*continue/i.test(ta)) sa += 55;
      if (/save\s*(&|and)\s*continue/i.test(tb)) sb += 55;
      if (/^continue(\s+application)?$/i.test(ta)) sa += 40;
      if (/^next(\s*step)?$/i.test(ta)) sa += 35;
      if (/^review$/i.test(ta)) sa += 30;
      if (/^continue(\s+application)?$/i.test(tb)) sb += 40;
  ...
  function isMarkAppliedCta(text) {
    ...
    if (/^mark\s*as\s*applied\b/i.test(t)) return true;
    if (/^applied\s*successfully\b/i.test(t)) return true;
    return false;
  }

  function isOpenApplicationCta(text) {
    var t = String(text || '').replace(/\s+/g, ' ').trim();
    if (!t) return false;
    if (/application\s*issue|blocked|upgrade|auto-?apply\s*with\s*ai/i.test(t)) return false;
    return /^open\s*application$/i.test(t) || /\bopen\s*application\b/i.test(t);
  }
  ...
  function isJobPoolHubPage(doc, url, opts) {
    ...
    if (/\/fill-apply/i.test(url)) return true;
    if (
  ...
  global.FillApplySynonyms = {
    ...
    LOCATION_FILTER_CHIP_CTA: LOCATION_FILTER_CHIP_CTA,
    ...
    isNavigationChrome: isNavigationChrome,
    JOBPOOL_NAV_CHROME_CTA: JOBPOOL_NAV_CHROME_CTA,
    ...
    isOpenApplicationCta: isOpenApplicationCta,
