...
(function (global) {
  ...
  function normalizeRecord(partial) {
    ...
    var ts = now();
    var fieldType = partial.fieldType || 'text';
    if (C && C.isValidFieldType && !C.isValidFieldType(fieldType)) fieldType = 'text';
    var display =
      C && C.normalizeValue
    ...
    return {
      id: partial.id || uuid(),
      canonicalKey: String(partial.canonicalKey || '').trim(),
      fieldType: fieldType,
      value: display,
      displayValue: display,
      aliases: Array.isArray(partial.aliases)
        ? partial.aliases
            .map(function (a) {
            ...
            .filter(Boolean)
        : [],
      source: partial.source || 'user_explicit',
      confidence: typeof partial.confidence === 'number' ? Math.max(0, Math.min(1, partial.confidence)) : 0.85,
  ...
  /**
   * Compact snapshot for injection into the fill context.
   */
  ...
  function importSnapshot(snapshot) {
    ...
    records.forEach(function (rec) {
      if (!rec || !rec.id || !rec.canonicalKey) return;
