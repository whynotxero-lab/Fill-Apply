...
(function (global) {
  ...
  async function getProfileFromBackend() {
    // Always local. JobPool (and other board hubs) are NOT a profile API —
    // fetching /profile after Load from JobPool returned HTML and broke fills.
    if (global.FillApplyProfile) return global.FillApplyProfile.getProfile();
    return null;
  }

  async function getDocumentsFromBackend() {
    // Always local chrome.storage documents. Never hit JobPool /documents
    // (returns the Next.js HTML shell → "Unexpected token <" / HTML-instead-of-JSON).
    if (Storage() && Storage().getDocuments) {
