...
(function (global) {
  ...
  var AUTH_RESULTS = {
    ...
    EMAIL_PASSWORD_AVAILABLE: 'EMAIL_PASSWORD_AVAILABLE',
  ...
  function inspectAuthPage(doc, profile, opts) {
    ...
    if (wall && wall.challenged) {
      ...
      // Plain email+password Create Account / login: when Environment or profile
      // supplies a password, do NOT pause — let signup-login / adapter fill.
      var canEmailPw = false;
      try {
        if (global.FillApplySignupLogin && global.FillApplySignupLogin.profileHasCredentials) {
          canEmailPw = !!global.FillApplySignupLogin.profileHasCredentials(profile);
        } else if (global.FillApplyAuthWalls && global.FillApplyAuthWalls.canAutoFillCredentials) {
          canEmailPw = !!global.FillApplyAuthWalls.canAutoFillCredentials(profile);
        }
      } catch (_cred) {
        canEmailPw = false;
      }
      var pwCount = wall.passwordFields != null ? wall.passwordFields : 0;
      var plainEmailPasswordSignup =
        canEmailPw &&
        pwCount >= 1 &&
        !isContinueApplying;
      if (plainEmailPasswordSignup) {
        return {
          result: AUTH_RESULTS.EMAIL_PASSWORD_AVAILABLE || 'EMAIL_PASSWORD_AVAILABLE',
          pause: false,
          detail: 'Email/password credentials available — autofill signup/login (no Google required)',
          action: 'fill_email_password',
