...
(function (global) {
  ...
  /** Preferred applicant display names (equivalent variants). */
  var APPLICANT_NAME_PREFERRED = 'Chaudhry Zahid Ali';
  var APPLICANT_NAME_VARIANTS = ['Chaudhry Zahid Ali', 'Chaudary Zahid Ali', 'Chaudhary Zahid Ali'];
  var APPLICANT_NAME_PARTS = ['chaudhry', 'chaudary', 'chaudhary', 'zahid', 'ali'];
  ...
  function namesEquivalent(a, b) {
    ...
    // Token-set equality (order-insensitive) for "Zahid Ali Chaudhry" etc.
  ...
  function preferredNamesFromProfile(profile) {
    ...
    if (profile) {
      var full =
        ...
        '';
      if (full) out.unshift(String(full));
      if (profile.firstName && profile.lastName) {
        out.push(String(profile.firstName) + ' ' + String(profile.lastName));
      }
    }
    var seen = {};
    var uniq = [];
  ...
  function inspectAuthPage(doc, profile, opts) {
    ...
    if (wall && wall.challenged) {
      if (googleActions.length) {
        return {
          ...
          wall: { markers: wall.markers, passwordFields: wall.passwordFields },
          href: href
        };
      }
      return {
        ...
        pause: true,
        detail: wall.detail || 'Auth wall without Google OAuth — complete manually',
        wall: { markers: wall.markers, passwordFields: wall.passwordFields },
        href: href
      };
    }
    ...
    if (googleActions.length && /sign\s*in|log\s*in|register|create\s*(an\s*)?account/i.test(pageText(doc))) {
      return {
        result: 'GOOGLE_AUTH_AVAILABLE',
        pause: false,
  ...
  function performAuthAction(doc, profile, inspection) {
    ...
    if (inspection.action === 'click_google') {
      var actions = findGoogleAuthActions(doc);
