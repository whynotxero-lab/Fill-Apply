...
(function (global) {
  ...
  function resolveCredentials(profile) {
    ...
    // Runner may stamp Environment password onto the profile for content-script inject
    // (memory starts empty after script inject until load()).
    if (isBlank(password)) {
      password = pick('__registrationPassword', 'registrationPassword');
      if (!isBlank(password)) passwordConfirm = password;
    }
  ...
  function findSocialContinueButtons(doc) {
    doc = doc || (typeof document !== 'undefined' ? document : null);
    if (!doc) return [];
    var nodes = doc.querySelectorAll(
      'a, button, [role="button"], div[role="button"], span[role="link"], input[type="button"]'
    );
    var out = [];
    for (var i = 0; i < nodes.length; i++) {
      var el = nodes[i];
      ...
      var dataProvider = String(
        el.getAttribute('data-provider') || el.getAttribute('data-social') || ''
      ).toLowerCase();
      var blob = (t + ' ' + aria + ' ' + dataProvider).trim();
      if (!blob) continue;
      var provider = '';
      if (/^continue as\b/i.test(t) || /continue as\b/i.test(t)) provider = 'continue_as';
      else if (
        /continue with google|sign in with google|sign up with google|register with google|^google$/i.test(
          blob
        ) ||
        dataProvider === 'google'
      )
        ...
        /continue with apple|sign in with apple|sign up with apple|register with apple|^apple$/i.test(blob) ||
        dataProvider === 'apple'
      )
        provider = 'apple';
      else if (
        /continue with facebook|sign in with facebook|sign up with facebook|register with facebook|^facebook$|^fb$/i.test(
          blob
        ) ||
        dataProvider === 'facebook'
      )
        provider = 'facebook';
      else if (/continue with linkedin|sign in with linkedin|sign up with linkedin|^linkedin$/i.test(blob))
        provider = 'linkedin';
      else if (/continue with indeed|sign in with indeed|sign up with indeed|^indeed$/i.test(blob))
        provider = 'indeed';
      else if (
        /sign\s*up\s+with\s+email|register\s+with\s+email|create\s+(an\s+)?account\s+with\s+email|sign\s*up\s+via\s+email/i.test(
          blob
        )
      )
        provider = 'email';
      else continue;
      out.push({ el: el, text: t || aria || provider, provider: provider });
    }
    // Prefer Continue as → Google → Apple → Facebook → LinkedIn → Indeed → Email
    var order = {
      continue_as: 0,
      google: 1,
      apple: 2,
    ...
    out.sort(function (a, b) {
      return (order[a.provider] != null ? order[a.provider] : 9) - (order[b.provider] != null ? order[b.provider] : 9);
    });
    return out;
  }

  function tryPreferSocialContinue(doc) {
    var list = findSocialContinueButtons(doc);
    if (!list.length) return { clicked: false, reason: 'no_social_cta' };
    // Prefer AtsAuth Google finder when available (higher confidence)
    try {
      var A = global.FillApplyAtsAuth;
      if (A && typeof A.findGoogleAuthActions === 'function') {
        var g = A.findGoogleAuthActions(doc);
        if (g && g.length && g[0].el) {
          try {
            g[0].el.click();
            return { clicked: true, text: g[0].text || 'Continue with Google', provider: 'google' };
          } catch (_g) {}
        }
      }
      if (A && typeof A.listGoogleAccountOptions === 'function') {
        var c = A.listGoogleAccountOptions(doc);
        if (c && c.length && c[0].el) {
          try {
            c[0].el.click();
            return {
              clicked: true,
              text: buttonText(c[0].el) || c[0].name || 'Continue as',
              provider: 'continue_as'
            };
          } catch (_c) {}
        }
      }
    ...
    try {
      pick.el.click();
      return { clicked: true, text: pick.text, provider: pick.provider };
    } catch (_e) {
      return { clicked: false, reason: 'click_failed' };
    }
  }
  ...
  async function prepareSignupOrLogin(doc, profile, opts) {
    opts = opts || {};
    doc = doc || (typeof document !== 'undefined' ? document : null);
    // Content-script inject: ensure Environment password is loaded from chrome.storage.
    try {
      var EnvLoad = global.FillApplyEnvironment;
      if (EnvLoad && typeof EnvLoad.load === 'function') {
        await EnvLoad.load();
      }
    } catch (_el) { /* ignore */ }
    var creds = resolveCredentials(profile);
    ...
    // Prefer social Continue (Google / Continue as / LinkedIn / Indeed) when visible.
    var social = tryPreferSocialContinue(doc);
    result.socialContinue = social;
    if (social && social.clicked) {
      result.ok = true;
      result.detail = 'Clicked social continue: ' + (social.text || social.provider || 'social');
      if (opts.waitMs) {
        await new Promise(function (r) {
          setTimeout(r, opts.waitMs);
        });
      }
      // If still on the same signup popup after social click, pause here (no tab hop).
      var stillWall = shouldPauseForAuth(doc, profile);
      var stillSocial = findSocialContinueButtons(doc);
      if (
        (stillWall && stillWall.pause) ||
        (stillSocial && stillSocial.length && !profileHasCredentials(profile))
      ) {
        result.pause = true;
        result.pauseReason = 'auth_wall';
        result.detail =
          'Signup popup still open after social continue — paused on same popup';
        result.ok = false;
        return result;
      }
      return result;
    ...
    if (!creds.hasPassword) {
      // Social unavailable and no password — pause on same popup when wall present
      if (gate && gate.wall && gate.wall.challenged) {
        result.pause = true;
        result.pauseReason = 'auth_wall';
        result.detail = gate.detail || 'Sign in / register required — paused on same popup';
        result.ok = false;
        return result;
      }
  ...
  global.FillApplySignupLogin = {
    ...
    findSocialContinueButtons: findSocialContinueButtons,
    tryPreferSocialContinue: tryPreferSocialContinue,
