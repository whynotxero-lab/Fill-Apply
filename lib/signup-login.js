...
(function (global) {
  ...
  function findSocialContinueButtons(doc) {
    doc = doc || (typeof document !== 'undefined' ? document : null);
    if (!doc) return [];
    var nodes = doc.querySelectorAll('a, button, [role="button"], div[role="button"], span[role="link"]');
    var out = [];
    for (var i = 0; i < nodes.length; i++) {
      var el = nodes[i];
      if (!isVisible(el)) continue;
      ...
      else if (/continue with google|sign in with google|sign up with google|^google$/i.test(t))
        provider = 'google';
      else if (/continue with linkedin|sign in with linkedin|sign up with linkedin|^linkedin$/i.test(t))
        provider = 'linkedin';
      else if (/continue with indeed|sign in with indeed|sign up with indeed|^indeed$/i.test(t))
        provider = 'indeed';
      else continue;
      out.push({ el: el, text: t, provider: provider });
    }
    // Prefer Continue as → Google → LinkedIn → Indeed
    var order = { continue_as: 0, google: 1, linkedin: 2, indeed: 3 };
