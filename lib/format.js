...
(function (global) {
  ...
  var EDUCATION_LEVELS = [
    ...
    },
    {
      ...
      match: /\b(master'?s?|mba|m\.?b\.?a|m\.?sc|m\.?s\b|m\.?a\b|m\.?com|post.?graduate|postgraduate|graduate degree)\b/,
      spellings: [
        "Master's Degree", "Masters Degree", "Master's", "Masters", "Master",
        "MBA", "Postgraduate", "Post Graduate", "Graduate Degree",
    ...
    },
    {
      ...
      match: /\b(bachelor'?s?|under.?graduate|b\.?sc|b\.?s\b|b\.?a\b|b\.?com|b\.?e\b|b\.?tech|licence|licenciatura)\b/,
      spellings: [
...
function fieldKind(target, key) {
    ...
    if (/\b(nationality|citizenship|citizen of)\b/.test(blob)) return 'country';
  ...
  function dateOrderFrom(hint) {
    ...
    if (
      /\b(birth\s*day|dob\s*day|day of birth)\b/.test(p) &&
      !/\b(year|month|yyyy|mm\/)\b/.test(p)
    ) {
      return 'day';
    }
    return null;
  ...
  function detectDateFormat(target, context) {
    ...
    if (/\b(birth_?year|dob_?year|yearofbirth|bday-year|bday_year)\b/.test(nameId)) {
      return 'year';
    }
    if (/\b(birth_?month|dob_?month|monthofbirth|bday-month|bday_month)\b/.test(nameId)) {
      return 'month';
    }
    if (/\b(birth_?day|dob_?day|dayofbirth|bday-day|bday_day)\b/.test(nameId)) {
      return 'day';
    }
   ...
   * ("Chaudhary Zahid Ali" → Last="Zahid Ali", not "Ali").
   */
  ...
  function valueVariants(value, kind) {
    ...
    if (country) {
      // Nationality selects list the demonym; country selects list the country.
      // Which one this control wants is unknown here, so offer both.
