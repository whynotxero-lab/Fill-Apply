...
function fieldKind(target, key) {
    ...
    if (
      /\b(country code|dial code|calling code|phone country|area code)\b/.test(blob) ||
      /country\s*\/\s*region\s*code|country[_\s-]?region[_\s-]?code/i.test(blob)
    ) {
      return 'phoneCountry';
    }
    ...
    if (key === 'graduationYear' || keyNorm === 'graduation_year') return 'graduationYear';
   ...
   */
  /**
   * True when a string looks like a dumped "City, City ZIP, Country, …" blob
   * rather than a real street Address Line 1.
   */
  function looksLikeComposedAddressJunk(s) {
    var t = trim(s);
    if (!t) return false;
    var commas = (t.match(/,/g) || []).length;
    if (commas >= 2 && /\b(saudi arabia|united arab emirates|uae|pakistan|qatar|kuwait)\b/i.test(t)) {
      return true;
    }
    // Repeated city / zip tokens: "Riyadh, Riyadh 12791, …"
    var parts = t.split(/,\s*/).map(function (x) { return lower(x).replace(/\s+/g, ' ').trim(); });
    var seen = {};
    for (var i = 0; i < parts.length; i++) {
      var tok = parts[i].replace(/\d+/g, '').trim();
      if (tok && seen[tok]) return true;
      if (tok) seen[tok] = 1;
    }
    return false;
  }
  ...
  function composeAddress(profile, mode) {
    profile = profile || {};
    var rawLine1 = trim(profile.addressLine1 || profile.street || '');
    // Never treat a full composed address blob as Address Line 1.
    if (!rawLine1 && profile.address && !looksLikeComposedAddressJunk(profile.address)) {
      rawLine1 = trim(profile.address);
    }
    if (looksLikeComposedAddressJunk(rawLine1)) rawLine1 = '';
    var line1 = rawLine1;
    var line2 = trim(profile.addressLine2 || '');
    if (looksLikeComposedAddressJunk(line2)) line2 = '';
  ...
  function parseBucket(label) {
    ...
    // Workable-style "+15 years" / "+15" (plus BEFORE the number)
    var plusBefore = t.match(/^[+＋]\s*(\d+(?:\.\d+)?)|(?:^|\s)[+＋]\s*(\d+(?:\.\d+)?)\s*(?:years?|yrs?)?\b/);
    if (plusBefore) {
      var pb = plusBefore[1] != null ? plusBefore[1] : plusBefore[2];
      return { min: Number(pb), max: Infinity };
    }
  ...
  function matchBucketIndex(labels, value) {
    ...
    if (n == null) return -1;
    var raw = trim(value);
    var wantsPlus = /\+|or more|and above|plus\b/i.test(raw);
    var bestIdx = -1;
    ...
    for (var i = 0; i < labels.length; i++) {
      ...
      if (bucket.max === Infinity && bestOpenIdx === -1) bestOpenIdx = i;
      if (bestIdx === -1 || width < bestWidth) {
        bestIdx = i;
        bestWidth = width;
      }
    }
    // Profile "15+" / "15 or more" → prefer open-ended "+15 years" over closed "10–15".
    if (wantsPlus && bestOpenIdx !== -1) return bestOpenIdx;
    return bestIdx;
  }
   ...
   */
  /**
   * True when option text is a known false-positive for short ISO codes
   * (e.g. ISO "SA" inside "American Samoa", not Saudi Arabia).
   */
  function isTerritoryFalsePositive(optionText, want) {
    var o = lower(optionText);
    var w = lower(want);
    if (!o || !w) return false;
    // American Samoa (AS, +1684) must never win for Saudi Arabia (SA, +966).
    if (/american\s*samoa|\b1684\b|\(+1684\)/.test(o)) {
      if (
        w === 'sa' ||
        w === 'sau' ||
        w === 'saudi' ||
        /saudi|966|ksa|kingdom of saudi/.test(w) ||
        w === '+966' ||
        w === '966'
      ) {
        return true;
    ...
    // Short ISO / dial tokens must not substring-match unrelated names.
    if (w.length <= 3 && /^[a-z0-9+]+$/.test(w)) {
      // Allow only if option is exactly the code, or code appears as a token / dial.
      var tokenOk =
        new RegExp(
          '(^|[^a-z0-9])' + w.replace(/[+]/g, '\\+') + '([^a-z0-9]|$)',
          'i'
        ).test(optionText) ||
        lower(optionText).trim() === w;
      if (!tokenOk) return true;
    }
    return false;
  }

  function optionContainsVariant(optionText, want) {
    var o = lower(optionText);
    var w = lower(want);
    if (!o || !w || w.length < 2) return false;
    if (isTerritoryFalsePositive(optionText, want)) return false;
    // Short codes (ISO2, tiny dial): token / dial-boundary only — never raw substring
    // ("sa" inside "american samoa").
    if (w.length <= 3) {
      if (o === w) return true;
      if (new RegExp('(^|[^a-z0-9])' + w.replace(/[+]/g, '\\+') + '([^a-z0-9]|$)').test(o)) {
        return true;
      }
      // Dial forms: (+966), +966, 966-
      if (/^\+?\d{1,4}$/.test(w)) {
        if (o.indexOf('+' + w.replace(/^\+/, '')) !== -1) return true;
        if (new RegExp('\\(+' + w.replace(/^\+/, '') + '\\)').test(o)) return true;
      }
      return false;
    }
  ...
  function matchOptionIndex(labels, value, kind) {
    ...
    for (var v = 0; v < variants.length; v++) {
      ...
      for (var i = 0; i < list.length; i++) {
        if (isTerritoryFalsePositive(list[i], variants[v])) continue;
        if (lower(list[i]) === wanted) return { index: i, how: 'variant_exact' };
      }
    }

    // Country-like: prefer dial-code / full-name contains before short ISO.
    if (countryish) {
      var longFirst = variants.slice().sort(function (a, b) {
        return String(b).length - String(a).length;
      });
      for (var vL = 0; vL < longFirst.length; vL++) {
        var wantL = longFirst[vL];
        if (String(wantL).length < 4 && !/^\+?\d{1,4}$/.test(String(wantL))) continue;
        for (var jL = 0; jL < list.length; jL++) {
          if (optionContainsVariant(list[jL], wantL)) {
            return { index: jL, how: 'variant_contains' };
          }
        }
      }
    }
    ...
    for (var v2 = 0; v2 < variants.length; v2++) {
      var want2 = variants[v2];
      if (String(want2).length < 2) continue;
      for (var j = 0; j < list.length; j++) {
        if (optionContainsVariant(list[j], want2)) {
          return { index: j, how: 'variant_contains' };
        }
      }
    }
  ...
  function formatForField(target, value, context) {
    ...
    switch (kind) {
      ...
      case 'url':
        ...
        break;
      case 'graduationYear': {
        // Bare year field — accept YYYY only; never degree strings like "MBA".
        var gy = String(value == null ? '' : value).trim();
        var gyMatch = gy.match(/\b((?:19|20)\d{2})\b/);
        if (/[a-zA-Z]{2,}/.test(gy) && !gyMatch) {
          result = { value: '', skip: true, format: 'reject_non_date' };
        } else if (gyMatch) {
          result = { value: gyMatch[1], skip: false, format: 'year' };
        } else {
          result = { value: '', skip: true, format: 'blank' };
        }
        break;
      }
      case 'date':
        // Year/date controls must never receive degree strings like "MBA".
        if (/[a-zA-Z]{2,}/.test(String(value)) && !/\d{4}/.test(String(value))) {
          result = { value: '', skip: true, format: 'reject_non_date' };
        } else {
  ...
  global.FillApplyFormat = {
    ...
    looksLikeComposedAddressJunk: looksLikeComposedAddressJunk,
