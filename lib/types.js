...
*   companion: navigate-only (Simplify companion) — settle → Continue/Next → grace; never fill/upload
...
(function (global) {
  ...
  const RUN_MODES = ['register', 'fill', 'navigate', 'ready', 'submit', 'companion'];
  ...
  /** @deprecated Prefer JOBPOOL_FILL_APPLY_URL — legacy Applications hub. */
  var JOBPOOL_APPLICATIONS_URL = JOBPOOL_DEFAULT_BASE_URL + '/applications';
  /** Primary JobPool hub for Fill & Apply (Open Application → Applied Successfully). */
  var JOBPOOL_FILL_APPLY_URL = JOBPOOL_DEFAULT_BASE_URL + '/fill-apply';
  /** Alias used by Load / Open JobPool UI — always the Fill-Apply queue. */
  var JOBPOOL_HUB_URL = JOBPOOL_FILL_APPLY_URL;
  ...
  const DEFAULT_RUN_CONFIG = {
    ...
    focusHud: true,
    /**
     * Simplify companion (runMode=companion): navigate-only.
     * Wait for host-page DOM quiet (settleMs), click Continue/Next, grace for Simplify.
     */
    simplifyCompanion: false,
    companionSettleMs: 12000,
    companionMaxWaitMs: 12 * 60 * 1000,
    companionGraceMs: 4000
  };
