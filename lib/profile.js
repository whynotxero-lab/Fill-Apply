...
(function (global) {
  ...
  /**
   * Built-in Zahid applicant (private zip 1.26.6+). When FillApplyPrivateZahidSeed
   * is loaded, its profileFields() are the source of truth; otherwise this
   * minimal identity shell still beats Mock so Start works after Load unpacked.
   * Create / Reset Zahid restores the seed (not an empty shell).
   */
  const ZAHID_GENERAL_PROFILE_FALLBACK = {
    name: 'Zahid',
    salutation: 'Mr.',
    title: 'Mr.',
    firstName: 'Chaudhry',
    middleName: '',
    lastName: 'Zahid Ali',
    preferredName: 'Chaudhry Zahid Ali',
    fullName: 'Chaudhry Zahid Ali',
    headline: 'CMA, ACCA — Strategic Finance & FP&A Professional',
    email: 'czahidali.accacma@gmail.com',
    emailConfirm: 'czahidali.accacma@gmail.com',
    password: 'Zahid@Finance786',
    passwordConfirm: 'Zahid@Finance786',
    confirm_password: 'Zahid@Finance786',
    retype_password: 'Zahid@Finance786',
    phone: '504131857',
    phoneCountry: '+966',
    ...
    remotePreference: 'Onsite for KSA, otherwise Remote',
    ...
    yearsExperience: '15+',
    highestEducation: 'MBA Executive',
    school: 'Virtual University of Pakistan',
    degree: 'MBA Executive',
    fieldOfStudy: 'Finance',
    graduationYear: '2018',
    gpa: '3.75',
    certifications: 'ACCA (UK) 2005; CMA (IMA USA) 2015',
    skills: 'FP&A, Budgeting, Forecasting, Financial Reporting, ACCA, CMA',
    ...
    requiresSponsorship: 'No',
    resumeSummary:
      'CMA, ACCA — Strategic Finance & FP&A Professional with 15+ years across KSA and regional finance leadership.',
    workHistory: '',
    education:
      'MBA Executive Finance, Virtual University of Pakistan, 2018, CGPA 3.75; B.Com University of the Punjab Lahore 2001; ACCA (UK) 2005; CMA (IMA USA) 2015',
    coverLetter: '',
    experienceEntries: [],
    educationEntries: [
      {
      ...
      },
      {
        school: 'IMA (USA)',
        degree: 'CMA',
        fieldOfStudy: 'Management Accounting',
        end: '2015',
        location: 'United States'
      }
    ],
    customQA: [
      { question: 'Relatives at company', answer: 'No' },
      { question: 'Former employee', answer: 'No' },
      { question: 'Work authorization without permit', answer: 'Yes' },
      { question: 'Led or supervised teams', answer: 'Yes' },
      { question: 'Finance/accounting qualifications', answer: 'Yes' },
      { question: 'Entertainment industry objections', answer: 'No' },
      { question: 'How did you hear about this position?', answer: 'Job Board' },
      { question: 'Are you a member of the Al Futtaim Schools alumni?', answer: 'Not Applicable' },
      { question: 'Are you a qualified Chartered Accountant (CA)?', answer: 'No' },
      { question: 'Are you ACCA qualified?', answer: 'Yes' },
      { question: 'Are you based in the UAE?', answer: 'No' }
    ],
    customAnswers: {
      'Relatives at company': 'No',
      'Former employee': 'No',
      'Work authorization without permit': 'Yes',
      'Led or supervised teams': 'Yes',
      'Finance/accounting qualifications': 'Yes',
      'Entertainment industry objections': 'No',
      ...
      'Are you ACCA qualified?': 'Yes',
      'Are you based in the UAE?': 'No',
      'Located in UAE': 'No'
    }
  };

  function zahidSeedFields() {
    try {
      if (global.FillApplyPrivateZahidSeed && typeof global.FillApplyPrivateZahidSeed.profileFields === 'function') {
        var seeded = global.FillApplyPrivateZahidSeed.profileFields();
        if (seeded && typeof seeded === 'object' && seeded.email) {
          return Object.assign({ name: 'Zahid' }, seeded);
        }
      }
    } catch (_e) { /* ignore */ }
    return Object.assign({}, ZAHID_GENERAL_PROFILE_FALLBACK);
  }

  /** @deprecated alias — use zahidSeedFields() */
  var ZAHID_GENERAL_PROFILE = ZAHID_GENERAL_PROFILE_FALLBACK;
  ...
  /** Prefer Zahid (built-in private seed) over Mock when picking a fallback active id. */
  function preferDefaultActiveId(profiles, currentActiveId) {
    // Honor explicit selection (including Mock) — demotion happens in ensureZahidProfile({activate:true}) / install seed.
  ...
  function isLockedProfile(record) {
    ...
    // Zahid is the built-in seed but remains editable / renameable / importable.
    if (isZahidName(record.name) || String(record.id || '') === 'zahid') return false;
  ...
  function ensureMigrated() {
    ...
    migratePromise = (async function () {
      ...
      var seedSrc = zahidSeedFields();
      // Legacy single profile that already looks like Zahid → keep fields
      if (
        legacy &&
        typeof legacy === 'object' &&
        (legacy.email || legacy.firstName || legacy.fullName) &&
        (/zahid/i.test(String(legacy.fullName || '')) ||
          /czahidali/i.test(String(legacy.email || '')) ||
          /zahid/i.test(String(legacy.lastName || '')))
      ) {
        seedSrc = Object.assign({}, seedSrc, legacy);
      }
      var zahidRecord = Object.assign({}, fieldsFromInput(seedSrc), {
        id: 'zahid',
        name: 'Zahid',
        locked: false,
        systemProfile: false,
      ...
      // Fresh init: Zahid is the sole built-in active profile (private seed)
  ...
  async function readStore() {
    await ensureMigrated();
    // Ensure Zahid seed exists. Activate only when nothing valid is active (fresh / broken).
    try {
      var raw = await readStoreRaw();
      var needActivate = !raw.activeId || !raw.profiles[raw.activeId];
      await ensureZahidProfile({ activate: needActivate, reset: false });
    } catch (_eZahid) {
  ...
  async function saveProfile(profile) {
    ...
    var now = Date.now();
    var keepMockLock = isLockedProfile(existing) && (isMockName(existing.name) || String(existing.id) === 'mock');
    store.profiles[store.activeId] = Object.assign({}, fields, {
      id: existing.id,
      name: keepMockLock ? 'Mock' : existing.name,
      locked: keepMockLock,
      systemProfile: keepMockLock,
      builtInSeed: !!existing.builtInSeed || isZahidName(existing.name) || String(existing.id) === 'zahid',
  ...
  async function ensureZahidProfile(opts) {
    ...
    var now = Date.now();
    var zahidId = findZahidId(profiles);
    var seedSrc = opts.fields && typeof opts.fields === 'object' ? opts.fields : zahidSeedFields();
    var fields = fieldsFromInput(seedSrc);
    var changed = false;
    if (!zahidId) {
      zahidId = 'zahid';
      ...
      // Normalize display name Zahid General → Zahid; fill empty identity from seed
      var cur = profiles[zahidId];
      if (cur) {
        var patched = Object.assign({}, cur);
        var needWrite = false;
        if (String(cur.name || '') === 'Zahid General') {
          patched.name = 'Zahid';
          needWrite = true;
        }
        if (!String(cur.email || '').trim() && fields.email) {
          Object.keys(fields).forEach(function (k) {
            if (patched[k] == null || String(patched[k]).trim() === '') {
              patched[k] = fields[k];
            }
          });
          needWrite = true;
        }
        if (!patched.builtInSeed) {
          patched.builtInSeed = true;
          needWrite = true;
        }
        if (patched.systemProfile) {
          patched.systemProfile = false;
          needWrite = true;
        }
        if (needWrite) {
          patched.updatedAt = now;
          profiles[zahidId] = patched;
          changed = true;
        }
      }
    }
    ...
    if (opts.activate) {
      ...
      activeId = preferDefaultActiveId(profiles, activeId) || zahidId;
      changed = true;
    }
    if (changed) {
  ...
  /**
   * Ensure Zahid (sole built-in active seed) exists and is active.
   * Mock may still exist for demos but is demoted — never the fresh-load active.
   * Import remains available for advanced use. Never wipe richer imported data
   * unless opts.resetZahid.
   */
  async function ensureDefaultProfiles(opts) {
    ...
    // Mock is optional / demoted — create only when explicitly requested
    if (opts.keepMock || opts.ensureMock) {
      await ensureMockProfile();
    }
    var store = await readStoreRaw();
    var mockId = findMockId(store.profiles);
    if (opts.forceZahidActive !== false && zahidId && store.profiles[zahidId]) {
      if (store.activeId !== zahidId) await writeStore(store.profiles, zahidId);
    } else if (!store.activeId || !store.profiles[store.activeId]) {
      if (zahidId && store.profiles[zahidId]) await writeStore(store.profiles, zahidId);
      else if (mockId && store.profiles[mockId]) await writeStore(store.profiles, mockId);
    }
  ...
  async function createZahidGeneralProfile() {
    // Explicit user action: reset Zahid to the private seed; Mock stays demoted if present.
    var zahidId = await ensureZahidProfile({ reset: true, activate: true, fields: zahidSeedFields() });
    var store = await readStoreRaw();
  ...
  global.FillApplyProfile = {
    ...
    ZAHID_GENERAL_PROFILE_FALLBACK: ZAHID_GENERAL_PROFILE_FALLBACK,
    zahidSeedFields: zahidSeedFields,
