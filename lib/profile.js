...
(function (global) {
  ...
  /**
   * Ensure Mock + Zahid shells exist. Non-destructive on updates:
   * never auto Create/Reset Zahid; never wipe imported/user data.
   * Activation only when opts.forceZahidActive or opts.fresh (explicit).
   * Create/Reset Zahid remains createZahidGeneralProfile() — user action only.
   */
  async function ensureDefaultProfiles(opts) {
    ...
    var zahidId = await ensureZahidProfile({
      activate: !!opts.forceZahidActive || !!opts.fresh,
    ...
    // Create missing Zahid shell only — do not reset existing data
    if (!findZahidId(store.profiles)) {
      zahidId = await ensureZahidProfile({
        activate: !!opts.forceZahidActive || !!opts.fresh || !store.activeId,
        reset: false
      });
    }
    // Explicit fresh / force only: switch active to Zahid; Mock stays
    if (opts.forceZahidActive || opts.fresh) {
      store = await readStoreRaw();
