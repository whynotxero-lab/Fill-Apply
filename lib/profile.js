...
(function (global) {
  ...
  function ensureMigrated() {
    ...
    migratePromise = (async function () {
      ...
      // Fresh init: Mock is the default active profile (Zahid empty shell secondary; import real data)
  ...
  async function readStore() {
    ...
    try {
      ...
      if (!findZahidId(raw.profiles)) {
        // Create empty Zahid shell without stealing active (Mock remains default)
        await ensureZahidProfile({ activate: false });
      }
    } catch (_eZahid) {
      /* ignore */
    }
  ...
  async function ensureDefaultProfiles(opts) {
    ...
    // Create missing Zahid shell only — do not reset existing data / do not steal active from Mock
    ...
    // Explicit Create/Reset Zahid: activate Zahid
    if (opts.forceZahidActive && zahidId && store.profiles[zahidId]) {
      ...
      // Fresh install: Mock is the default active profile
