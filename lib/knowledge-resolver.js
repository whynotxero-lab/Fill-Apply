...
(function (global) {
  ...
  function matchRecord(records, descriptor, profile) {
    ...
    var identified = C.matchCanonical(hay, {
      records: records,
      also: [descriptor && descriptor.placeholder, descriptor && descriptor.name],
    ...
    // Strong alias-only reuse: require high score AND same semantic key family.
    // Never bind via a single shared token (prevents salary/relocate collisions).
    var best = null;
    var bestScore = 0;
    records.forEach(function (rec) {
      ...
      // Gate: exact/near-exact only (≥ 100) — never weak single-token (≥ 22)
    ...
    if (best) {
      // Alias evidence may attach wording to an existing key; still require
      // that identified key equals the record key OR identified was derived.
  ...
  function resolve(profile, descriptor, map) {
    ...
    if (knowledgeHit && knowledgeHit.record && usable(knowledgeHit.record)) {
      ...
      packed.action = 'FILL';
      packed.canonicalKey = knowledgeHit.record.canonicalKey;
    ...
    if (legacy && !isBlank(legacy.value)) {
      // Guard: do not let legacy customAnswers cross semantic exclusions
      if (
        C &&
        C.isExcludedPair &&
        identified &&
        identified.key &&
        legacy.key &&
        C.isExcludedPair(identified.key, String(legacy.key))
      ) {
        legacy = null;
      }
    }
