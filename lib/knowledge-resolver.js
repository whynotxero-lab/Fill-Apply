 ...
 *   2. user profile (stable facts) — beats weak adaptive
 *   3. Adaptive Dictionary confirmed (user_confirmed)
 *   4. Question Bank (authored)
 *   5. built-in knowledge (FIELD_MAP / customAnswers / customQA)
...
(function (global) {
  ...
  function resolve(profile, descriptor, map) {
    ...
    // 2. Profile first (beats adaptive)
    if (identified && identified.key) {
      var idPkEarly =
      ...
      if (!idPkEarly) {
        ...
        if (profileValueForKey(profile, camelEarly)) idPkEarly = camelEarly;
        else if (profileValueForKey(profile, identified.key)) idPkEarly = identified.key;
        else if (identified.key === 'address' || identified.key === 'address_full' || identified.key === 'addressFull') {
          idPkEarly = identified.key === 'address_full' ? 'addressFull' : 'address';
        }
      }
      if (idPkEarly) {
