...
- **Consent ticked, EEO not invented** — agree/privacy/electronic-signature checkboxes are ticked so Next can proceed; other voluntary self-identification stays skipped/reported. Required fields with no answer are named in `missingRequired`.
- **Gender is select/radio only** — never free-text; Male maps to Male/Man/M options.
- **CV import first** — when the page offers Fill/Import with CV or Resume, that control is clicked before normal autofill.
- **Learn on human input** — unknown or corrected fields grow the adaptive dictionary (Key / Type / Value), including gender selects; Hilton and all ATS observe the same pipeline.
